...
private:
    ...
    std::atomic<int>  pendingTaskIndex{-1};
    std::string       pendingNodeName;
...
void TestClient::taskLoopFun(std::promise<void> &prom)
{
    ...
    const int n = static_cast<int>(poseList.size());
    std::cout << "task list num:" << n << std::endl;
    ...
    // Build a stable forward+reverse patrol order over INDICES.
    // poseList is never mutated, so qrPoseList[].taskIndex stays valid.
    std::vector<int> order;
    order.reserve(2 * n);
    for (int i = 0; i < n; ++i) order.push_back(i);
    for (int i = n - 2; i >= 1; --i) order.push_back(i); // bounce back, no dup endpoints

    size_t step = 0;
    while (threadControl)
    {
        int idx = order[step % order.size()];

        is_arrived = false;
        {
            std::lock_guard<std::mutex> lock(pendingMutex);
            pendingTaskIndex = idx;
            pendingNodeName.clear(); // set if SLAM service echoes a node name
        }

        int32_t statusCode = Call(
            ROBOT_API_ID_POSE_NAV_PL,
            poseList[idx].toJsonStr(),
            data);

        std::cout << "[NAV] -> waypoint " << idx
                  << " statusCode:" << statusCode << " data:" << data << std::endl;

        // Wait for arrival, with a timeout so a never-arriving goal can't hang.
        auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(60);
        while (!is_arrived && threadControl)
        {
            if (std::chrono::steady_clock::now() > deadline)
            {
                std::cout << "\033[31m[NAV] Timeout on waypoint " << idx
                          << ", skipping.\033[0m" << std::endl;
                break;
            }
...
void TestClient::slamKeyInfoHandler(const void *message)
{
    ...
    {
        // ===== DIAG 2: full task_result so we can see what targetNodeName contains =====
        std::cout << "\033[36m[DIAG-ARRIVAL] task_result = " << jsonData.dump()
                  << "\033[0m" << std::endl;

        std::string node = jsonData["data"].value("targetNodeName", std::string());
        bool arrived = jsonData["data"]["is_arrived"];

        // Only honor a result that matches the goal we're currently waiting on.
        // If the SLAM service doesn't echo a usable node name, pendingNodeName
        // stays empty and we fall back to accepting any result (timeout still guards).
        bool matches;
        {
            std::lock_guard<std::mutex> lock(pendingMutex);
            matches = pendingNodeName.empty() || (node == pendingNodeName);
        }

        if (!matches)
        {
            std::cout << "[NAV] Ignoring stale task_result for " << node << std::endl;
            return;
        }
        ...
        if (arrived)
        {
            std::cout << "I arrived " << node << std::endl;
        }
        else
        {
            std::cout << "I not arrived " << node
...
void TestClient::relocationPlFun()
{
    ...
    // ===== DIAG 1: raw relocalization response (look for any status/confidence field) =====
    std::cout << "\033[36m[DIAG-RELOC] raw response data = " << data << "\033[0m" << std::endl;

    if (statusCode == 0)
    {
        // Verify relocalization converged before trusting the frame.
        // Assumes you start on the physical dock mark, so the pose should
        // settle near the map origin (0,0). Adjust threshold to taste.
        ...
        // ===== DIAG 3: measured pose right after relocalization =====
        std::cout << "\033[36m[DIAG-POSE] after reloc: x=" << p.x << " y=" << p.y
                  << " z=" << p.z << " dist_from_origin=" << dist << "m\033[0m" << std::endl;

        if (dist > 0.5) // meters
        {
            std::cout << "\033[31m[WARN] Relocalization suspect: pose is "
                      << dist << "m from expected dock origin. "
                      << "Re-dock and retry before navigating. "
                      << "QR scanning NOT enabled.\033[0m" << std::endl;
            return;
        }

        std::cout << "[INFO] Relocation verified (" << dist
                  << "m from origin). QR scanning enabled." << std::endl;
        startQrScanner();
    }
}
...
void TestClient::qrScanLoop()
{
    ...
    while (qrThreadRunning)
    {
        ...
        if (!qrText.empty())
        {
            // update on EVERY visible frame, so the last visible pose is kept
